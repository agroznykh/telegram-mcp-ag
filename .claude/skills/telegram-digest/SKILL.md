...
Применять не только к «сводке непрочитанного» буквально: как только в задаче
есть чтение чата, где могут оказаться голосовые/кружки/аудио, — правильный
порядок вызовов (шаги 4-7 ниже: сначала проверить квоту, транскрибировать
пачкой, а не по одному сообщению в цикле, довести `pending` до конца) нужен
именно из-за этого, а не из-за слова «непрочитанное» в запросе пользователя.
...
Если инструменты есть, но любой вызов возвращает ошибку про отозванную
сессию (устройство удалили в Telegram → Настройки → Устройства, либо
разлогинили все сеансы сразу) — сервер сам это распознаёт и кладёт в текст
ошибки готовую команду для перелогина, подобранную под ОС этой машины (или
обе — если ОС определить не удалось). Показать пользователю эту команду
дословно, не пересказывая её своими словами и не сокращая до «сессия
истекла, зайдите заново» без самой команды.
